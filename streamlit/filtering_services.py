# streamlit/utils/services/filtering_services.py

import pandas as pd
from collections import Counter

# --- All your get_best_* helper functions go here ---
# (I'm omitting them for brevity, but they should be here, unchanged)
...
def get_best_sex(df, choice, dw, dm):
    """
    BUG FIX: This function now always returns a tuple of two items
    to avoid the 'too many values to unpack' error.
    """
    if choice == 0: # When "Doesn't Matter" is chosen
        security = dm.iloc[:, 1]
        max_row_security = dm.loc[security.idxmax()]
        max_alcaldia_security = max_row_security[dm.columns[0]]
        # Return a tuple with a placeholder 'None' for the second value
        return (max_alcaldia_security, None)
    elif choice == 2: # Female
        data = df.iloc[1:, 18]
        max_row = df.loc[data.idxmax()]
        max_alcaldia = max_row[df.columns[0]]
        security = dw.iloc[:, 1]
        ...
        # Failsafe for any other unexpected choice value
        return (None, None)
...
def get_best_filter(
    data_work, data_sport, data_school, data_religion, data_marriage, data_culture, data_budget, data_people, data_housing,
    ...
    # This line will now work correctly for all choices
...
def get_alcaldia_info(data_work,data_sport,data_school,data_religion,data_marriage,data_culture,data_budget,data_people,data_housing,\
    ...
    # No changes to other data preparations...
    work_result=data_work.iloc[1:,:].to_dict(orient="records")
    plot_info.update({"Work":work_result})

    sport_result=data_sport.to_dict(orient="records")
    plot_info.update({"Sports Centers":sport_result})
    ...
    plot_info.update({"Schools":school_result})
    ...
    plot_info.update({"Religion":religion_result})
    ...
    plot_info.update({"Age Distribution":ages_result})

    number_people_result=data_housing[["NOM_MUN","PROM_OCUP"]].iloc[1:,:].to_dict(orient="records")
    plot_info.update({"Household Size":number_people_result})

    quality_result=data_housing[["NOM_MUN","BUEN_EDO"]].iloc[1:,:].to_dict(orient="records")
    plot_info.update({"Housing Quality":quality_result})

    recreation_result=data_recreation.to_dict(orient="records")
    plot_info.update({"Recreational Spaces":recreation_result})
    
    # --- CHANGE HERE: Use the 'normalized' column for security scores ---
    security_women_result=data_security_women[["NOM_MUN","normalized"]].to_dict(orient="records")
    plot_info.update({"Security for Women":security_women_result})

    security_result=data_security_everyone[["NOM_MUN","normalized"]].to_dict(orient="records")
    plot_info.update({"General Security":security_result})
    # --- END CHANGE ---

    green_result=data_green[["Alcaldía","Superficie (m²)_PORCENTAJE"]].iloc[1:,:].to_dict(orient="records")
    plot_info.update({"Green Spaces":green_result})

    health_result=data_health[["Demarcación territorial","NUMERO_CENTROS_PORCENTAJE"]].iloc[1:,:].to_dict(orient="records")
    plot_info.update({"Health Centers":health_result})

    restaurant_result=data_restaurant.to_dict(orient="records")
    plot_info.update({"Restaurants":restaurant_result})
    ...
    plot_info.update({"Transportation":transportation_result})
...
if __name__ == "__main__":
    ...
    """filter=get_best_filter(data_work,data_sport,data_school,data_religion,data_marriage,data_culture,data_budget,data_people,data_housing,\
                           data_recreation,data_security_women,data_security_everyone,data_green,data_health,data_restaurant,data_transportation,\
                            0,0,2,1,1,1,1,6000000,0,1,1,1,30,0,1,0,0,0)
    print(filter)"""
